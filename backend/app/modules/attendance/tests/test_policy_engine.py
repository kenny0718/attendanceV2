...
# ============================================
# Phase 1 Timezone Fix Regression Tests
# P1-01 / P1-02 / P1-03
# ============================================

class TestTimezoneFixRegression:
    """Regression tests for P1-01, P1-02, P1-03 timezone bug fixes"""

    def test_p1_02_taipei_early_morning_punch_correct_date(self):
        """P1-02: Taipei 00:30 打卡 => UTC 前一天，業務日必須是 Taipei 當日

        場景: 2026-03-12 00:30 Asia/Taipei = 2026-03-11 16:30 UTC
        punch_in_time.date() (UTC) = 2026-03-11  <-- 舊 bug：用這個日期
        正確業務日 (Taipei)         = 2026-03-12  <-- 修正後應使用這個

        如果用舊 bug，expected_start = 2026-03-11 09:00 Taipei
        和 punch_in 2026-03-12 00:30 Taipei 比較，結果不遲到（差距為負）
        修正後 expected_start = 2026-03-12 09:00 Taipei，打卡 00:30 < 09:00，不遲到
        """
        from zoneinfo import ZoneInfo
        TZ_TAIPEI = ZoneInfo("Asia/Taipei")

        # 2026-03-12 00:30 Asia/Taipei => 2026-03-11 16:30 UTC
        punch_in = datetime(2026, 3, 11, 16, 30, 0, tzinfo=ZoneInfo("UTC"))
        punch_out = datetime(2026, 3, 11, 19, 0, 0, tzinfo=ZoneInfo("UTC"))  # 03:00 Taipei
        ...
        # punch_in = 00:30 Taipei, work_start = 09:00 Taipei on same Taipei date
        # 00:30 < 09:00, so NOT late
        assert result.is_late == False, (
            f"P1-02 regression: expected not late for Taipei 00:30 punch-in, "
            f"got late_minutes={result.late_minutes}"
        )

    def test_p1_01_utc_punch_no_8hr_offset_on_late_check(self):
        """P1-01: UTC punch_in 時，work_start_time 必須被解讀為 Asia/Taipei 時間

        場景: 打卡 2026-03-04 09:05 UTC = 17:05 Taipei
        work_start = 09:00 (Taipei)
        正確: expected_start = 2026-03-04 09:00 Taipei = 01:00 UTC
        punch_in UTC 09:05 > expected_start UTC 01:00 => 遲到 484 分鐘

        舊 bug: expected_start = 2026-03-04 09:00 UTC (被 replace 成 UTC)
        比較: 09:05 UTC vs 09:00 UTC => 遲到 5 分鐘 (錯誤結果)
        """
        from zoneinfo import ZoneInfo

        # 2026-03-04 09:05 UTC = 17:05 Taipei
        punch_in = datetime(2026, 3, 4, 9, 5, 0, tzinfo=ZoneInfo("UTC"))
        punch_out = datetime(2026, 3, 4, 18, 5, 0, tzinfo=ZoneInfo("UTC"))  # 02:05+1 Taipei
        ...
        policy = AttendancePolicy(
            ...
            company_id="company-a",
            name="Standard Policy",
            work_start_time=time(9, 0),   # 09:00 Asia/Taipei
            work_end_time=time(18, 0),
            grace_period_minutes=0,
            overtime_threshold_minutes=540
        )
        ...
        # punch_in = 09:05 UTC = 17:05 Taipei
        # expected_start = 09:00 Taipei = 01:00 UTC
        # 17:05 > 09:00 Taipei => late by 8*60+5 = 485 min
        assert result.is_late == True, "P1-01 regression: should be late"
        assert result.late_minutes == 485, (
            f"P1-01 regression: expected 485 late minutes, got {result.late_minutes}"
        )

    def test_p1_03_work_window_duration_minutes_timezone_safe(self):
        """P1-03: WorkWindow.duration_minutes() 不應依賴 server 時區

        datetime.today() 受 server 時區影響；改用 date.min 後
        duration_minutes() 在任何 server 時區下結果一致。
        """
        from app.modules.attendance.policy_engine import WorkWindow

        window = WorkWindow(start_time=time(8, 0), end_time=time(17, 0))
        assert window.duration_minutes() == 540, (
            f"P1-03 regression: expected 540 minutes for 08:00-17:00, "
            f"got {window.duration_minutes()}"
        )

        window2 = WorkWindow(start_time=time(0, 0), end_time=time(23, 59))
        assert window2.duration_minutes() == 1439

        window3 = WorkWindow(start_time=time(9, 30), end_time=time(18, 30))
        assert window3.duration_minutes() == 540

    def test_p1_02_split_shift_taipei_early_morning_correct_windows(self):
        """P1-02 split shift: 跨午夜 Taipei 打卡，window 邊界取 Taipei 日期

        場景: 2026-03-12 00:30 Taipei 打卡 (UTC 2026-03-11 16:30)
        window: 08:00-14:00 + 16:00-18:00 (Taipei)
        punch_out: 2026-03-12 03:00 Taipei (UTC 2026-03-11 19:00)

        window_start_dt 應以 Taipei 日期 2026-03-12 為基準
        punch_in 00:30 < window 08:00 => 0 分鐘貢獻（在第一個 window 之前）
        """
        from zoneinfo import ZoneInfo
        from app.modules.attendance.policy_engine import WorkWindow, WorkSchedule
        ...
        # 2026-03-12 00:30 Asia/Taipei => 2026-03-11 16:30 UTC
        punch_in = datetime(2026, 3, 11, 16, 30, 0, tzinfo=ZoneInfo("UTC"))
        # 2026-03-12 03:00 Asia/Taipei => 2026-03-11 19:00 UTC
        punch_out = datetime(2026, 3, 11, 19, 0, 0, tzinfo=ZoneInfo("UTC"))
        ...
        engine = AttendancePolicyEngine()
        result = engine.evaluate_with_schedule(session, schedule, None)

        # punch 00:30~03:00 Taipei does not overlap any window (08:00-14:00 or 16:00-18:00)
        assert result.work_minutes == 0, (
            f"P1-02 split shift regression: expected 0 work minutes for pre-window punch, "
            f"got {result.work_minutes}"
        )
