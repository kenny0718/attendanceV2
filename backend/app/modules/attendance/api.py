...
async def get_sessions_reporting(
    ...
    if sessions:
        ...
        display_name_map = {str(u.id): u.display_name for u in users}
    ...
    session_responses = [
        SessionResponse(
            ...
            duration_minutes=s.duration_minutes,
            status=s.status,
            display_name=display_name_map.get(str(s.user_id)),
        )
        for s in sessions
    ]
...
async def create_out_checkpoint(
    ...
    if checkpoint.gps_lat is not None and checkpoint.gps_lng is not None:
        from app.modules.attendance.schemas import GPSData
