<template>
  <PageCard>
    ...
    <div v-if="!company" class="detail-empty">
    ...
    <div v-else class="detail-body">
      ...
      <form class="detail-form" novalidate @submit.prevent="$emit('submit')">
        ...
        <div class="form-group">
          ...
          <select id="d-tz" :value="form.timezone" class="form-input" @change="$emit('update:form', { ...form, timezone: $event.target.value })">
        ...
        <div class="detail-actions">
          <button type="button" class="btn-toggle" :disabled="loading" @click="$emit('toggle-active')">
...
.btn-submit,
.btn-toggle { display: inline-flex; align-items: center; justify-content: center; gap: 8px; min-width: 144px; min-height: 46px; padding: 0 20px; border: none; border-radius: 16px; font-size: 15px; font-weight: 700; white-space: nowrap; cursor: pointer; }
...
@media (max-width: 900px) { .detail-meta { grid-template-columns: 1fr; } }
@media (max-width: 720px) { .detail-actions { flex-direction: column; align-items: stretch; } .btn-submit, .btn-toggle { width: 100%; } }
</style>
