  ...
  <div class="home-page">
    ...
    <div class="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <Card title="外出 / 返回" class="mb-6">
        ...
        <div class="mb-4">
          ...
          <div class="reason-chips flex flex-wrap gap-2 mb-3">
            <button
              ...
              :class="[
                'reason-chip px-3 py-1.5 rounded-full text-sm transition-all',
                breakOutReason === reason 
          ...
          <!-- 自訂原因 -->
          <div v-if="reasonCustoms.length > 0" class="custom-reasons flex flex-wrap gap-2 mb-3">
            <button
              v-for="reason in reasonCustoms"
              :key="reason"
              @click="selectReason(reason)"
              :class="[
                'reason-chip px-3 py-1.5 rounded-full text-sm transition-all flex items-center gap-1',
                breakOutReason === reason 
                  ? 'bg-secondary text-white shadow-md' 
                  : 'bg-blue-50 text-text-primary hover:bg-blue-100'
              ]"
            >
              {{ reason }}
              <span 
                @click.stop="removeCustomReason(reason)"
                class="remove-btn text-xs opacity-70 hover:opacity-100 ml-1"
              >
                ✕
              </span>
            </button>
          ...
          <input
            ...
            type="text"
            class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent mb-2"
            placeholder="請輸入外出原因"
            maxlength="50"
          />
        ...
        <!-- 外出/返回按鈕 -->
        <div class="punch-grid grid grid-cols-2 gap-4">
          <PunchButton 
            label="外出打卡"
            variant="secondary"
            icon="break"
        ...
        <!-- 提示訊息 -->
        <div class="punch-hint text-center text-sm text-text-secondary mt-4">
          <span v-if="todayStatus.is_on_break">目前外出中</span>
          <span v-else-if="todayStatus.punch_in && !todayStatus.punch_out">可進行外出打卡</span>
          <span v-else>請先完成上班打卡</span>
        </div>
        
        <!-- 外出 / 返回紀錄 -->
        <div v-if="breakPunches.length > 0" class="break-records mt-6 pt-6 border-t border-gray-200">
          <h4 class="text-sm font-medium text-text-primary mb-3">今日外出 / 返回紀錄</h4>
          <div class="space-y-2">
            <div
              v-for="punch in breakPunches.slice(0, 10)"
              :key="punch.punch_id"
              class="record-item flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors"
            >
              <div class="flex items-center gap-3 flex-1">
                <!-- 類型圖示 -->
                <div class="flex-shrink-0">
                  <svg v-if="punch.punch_type === 'break_start'" class="w-5 h-5 text-warning" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM7 9a1 1 0 000 2h6a1 1 0 100-2H7z" clip-rule="evenodd" />
                  </svg>
                  <svg v-else class="w-5 h-5 text-success" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div v-if="punch.notes" class="text-sm text-text-secondary mt-0.5 truncate">
                    {{ punch.notes }}
                  </div>
                </div>
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                  ...
                  <button
                    ...
                    class="text-xs text-text-secondary hover:text-text-primary px-2 py-1 rounded hover:bg-gray-200 transition-colors"
                    title="編輯原因"
                  >
      ...
      <div 
        ...
        class="success-toast fixed bottom-8 right-8 bg-green-600 text-white px-6 py-3 rounded-lg shadow-xl z-50"
      >
        ✓ {{ successMessage }}
      </div>
      ...
      <div 
        ...
        class="error-toast fixed bottom-8 right-8 bg-red-600 text-white px-6 py-3 rounded-lg shadow-xl z-50 max-w-md"
      >
...
// WP-11-13: 使用 shared location foundation
const {
