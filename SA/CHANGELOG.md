...
| 2026-04-14 11:25:04 +08:00 | Admin Companies detail / logo contract 對齊與 companies API 測試補強 | `done` | `tenants`, `frontend`, `admin companies`, `tests`, `SA index` | `yes` | `SA/CHANGELOG.md`, `frontend/src/views/admin/AdminCompaniesView.vue`, `frontend/src/components/admin/CompanyDetailPanel.vue`, `backend/app/modules/tenants/service.py`, `backend/app/modules/tenants/tests/test_companies_api.py` | 修正 admin companies detail 顯示邏輯以相容 detail contract，補上 company detail response shape 測試，並將 company logo upload 規格收斂為僅支援 PNG / JPG / JPEG、由後端生成 `logo_url`；更新對應 logo 測試後，`test_companies_api.py` 全數通過（22 passed）。 |
